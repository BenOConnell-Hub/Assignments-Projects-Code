...
def lin_value_calc(X, Y, x_line, y_line, x_label, y_label, g_title, errors_x, errors_y, y_at_zero,x_at_zero, y_max, x_max):
    ...
    print("")
    plt.text(0, -0.3, "R^2 = " + "{:.4f}".format(R2), size = 14)
    
    #Showing Graph
    plt.legend(title = 'Legend')
    plt.show()
    
x_values = np.array([0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2, 2.2, 2.4, 2.6, 2.8, 3, 3.2, 3.3])
y_values = np.array([1.43, 1.178, 0.938, 0.715, 0.455, 0.271, -0.055, -0.237, -0.517, -0.686, -0.919, -1.231, -1.415, -1.653, -1.957, -2.18, -2.45, -2.56])
    
lin_value_calc(x_values, y_values, 0, 0.06, 'Input Voltage (V)', 'Output Voltage (V)', 'Output vs Input Voltage of Addition Circuit', 0, 0, False, False, 0, 0)